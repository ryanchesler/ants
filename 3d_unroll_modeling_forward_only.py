...
class CFG:
    ...
    exp_name = 'forward_only'
    # starting_checkpoint = "/home/ryanc/kaggle/working/outputs/vesuvius_3d/pretrained/pretrained_model.pth"
    starting_checkpoint = "None"
    # starting_checkpoint = f"ants/working/outputs/vesuvius_unroll_3d/two_middle/vesuvius_unroll_3d-models/two_middle_3d_unet_last.pth"
    # ============== pred target =============
    target_size = 1
    # ============== model cfg =============
    model_name = '3d_unet'
        ...
        while invalid_volume:
            ...
            for depth, row in enumerate(history[:, -2:]):
                for xy_idx, point in enumerate(row):
                    ...
                    # image[point[1], point[0], depth+self.cfg.size//2] = 1
            ...
            image, final_label = image.astype(np.float16), drawn_forecast.astype(np.float16)
...
def train_fn(train_loader, model, criterion, optimizer, device):
    ...
    for step, (images, labels, all_mid, label_volume) in pbar:
        ...
        scaler.scale((loss/20000)+(volume_loss)).backward()
...
#     layer_num = []
#     layer_metrics = []
#     for i, error in enumerate(error_distances.mean(0)):
#         extrap_distance.append(i)
#         extrap_metrics.append(error)
#     for i, error in enumerate(error_distances.mean(1)):
#         layer_num.append(i)
#         layer_metrics.append(error)
